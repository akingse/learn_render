...
namespace eigen//eigen
{
    ...
    */
    ...
    inline Eigen::Matrix4f get_projection_matrix(float eye_fov, float aspect_ratio, float zNear, float zFar)
    {
		...
		float ty = -1.0f / tan(eye_fov * 0.5f); //field of view
        projection << 
    ...
    inline Eigen::Matrix4f get_orthographic_projection_matrix(const Eigen::AlignedBox3f& box)
    {
        return get_orthographic_projection_matrix(box.min().x(), box.max().x(), box.min().y(), box.max().y(), box.max().z(), box.min().z());
    }

    inline Eigen::Matrix4f get_projection_matrix(const Eigen::AlignedBox3f& box, float zNear, float zFar)
    {
        return get_orthographic_projection_matrix(box) * get_perspective_othographic_matrix(zNear, zFar);
    }
